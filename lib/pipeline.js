...
exports.importCards = async (context, sequence, options = {}) => {
	...
	for (const [ index, value ] of sequence.entries()) {
		...
		await Bluebird.map(step, async (segment, subindex, length) => {
			...
			if (_.get(segment, [ 'card', 'patch' ]) && segment.card.patch.length) {
				// If the patch doesn't update the origin, add it now
				if (!_.find(segment.card.patch, {
					path: '/data/origin'
				})) {
				...
				finalObject = evaluateObject(_.omit(segment.card, [ 'links' ]), references)
			} else {
			...
			const result = await context.upsertElement(finalObject.type, finalObject, {
