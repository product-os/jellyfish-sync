...
describe('pipeline.importContracts()', () => {
	test('should work with contract partials', async () => {
		...
		const sequence = [
			{
				...
				contract: {
					type: 'contract',
					slug: 'contract-78a1dfd7-21ea-405a-b269-de0b0e587975',
				},
			},
		];
		...
		const results = await pipeline.importContracts(context, sequence, {});

		expect(results.length).toBe(1);
		expect(upsertElementSpy.calledOnce).toBe(true);
		expect((upsertElementSpy.args as any)[0][0]).toBe('contract');
	...
	test('should work with JSONpatch', async () => {
		...
		const results = await pipeline.importContracts(
			context,
			sequence as any,
			{},
		);
