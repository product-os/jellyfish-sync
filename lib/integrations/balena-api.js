...
const mergeCardWithPayload = (preExistingCard, payload, resourceType, cardType) => {
	...
	const mirrorId = getMirrorId(payload.source, payload.payload.id, resourceType)
...
module.exports = class BalenaAPIIntegration {
	...
	async translate (event) {
		...
		// 3. Define the type of card that will be created
		// Define the target card
		let cardType = null

		// Define the source resource type
		let resourceType = null


		const { cardType, resourceType } = getCardAndResourceType(payload.resource)

		if ((cardType || resourceType) === undefined) {
