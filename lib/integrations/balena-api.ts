...
const getPreExistingCard = async (
...
const makeCard = (card: any, actor: any, time: string | number | Date) => {
	let date = new Date();
	if (time) {
		date = new Date(time);
	}
...
export const create = (options: SyncIntegrationOptions) => {
	return new BalenaAPIIntegration(options);
};
...
export const OAUTH_BASE_URL = integration.oauthBaseUrl;
...
export const whoami = async (
	...
	const { code: statusCode, body: externalUser } = await new Bluebird(
		(resolve, reject) => {
			request(
				{
					uri: `${OAUTH_BASE_URL}/user/v1/whoami`,
					headers: {
	...
	if (statusCode === 429 && retries > 0) {
		...
		return whoami(credentials, context as any, options, retries - 1);
	}
