...
const httpDiscourse = async (
	context: Context,
	integrationOptions: SyncIntegrationOptions,
	method: string,
	url: string,
	options: {
		baseUrl?: any;
...
const getDiscourseResource = async (
	context: Context,
	actor: any,
	options: any,
	baseUrl: any,
...
const getDiscourseEmailByUsername = async (
	context: Context,
	actor: string,
	options: any,
	baseUrl: string,
 ...
 */
const getDiscourseUserById = async (
	context: Context,
	actor: string,
	options: object,
	baseUrl: string,
 ...
 * @returns {(Object|Null)} Discourse user
 */
const getDiscourseUserByUsername = async (
	context: Context,
	actor: string,
	options: object,
	baseUrl: string,
 ...
 */
const getDiscourseTopicById = async (
	context: Context,
	actor: string,
	options: object,
	baseUrl: string,
 ...
 */
const getTopicId = (event: any): string => {
	if (!isEvent(event)) {
		if (event.topic_id) {
 ...
 */
const getTopicMirrorId = (baseUrl: string, event: object): string => {
 ...
 */
const getMirrorId = (baseUrl: string, event: any): string => {
...
const getRemoteTopicData = async (
	context: Context,
	actor: string,
	options: any,
	event: Card,
...
const processTopicData = async (
	...
	const newThreadCard: Partial<Card> = threadCard
		? Object.assign({}, _.cloneDeep(threadCard), {
				name: topicCard.name,
				active: topicCard.active,
		  })
		: _.cloneDeep(topicCard);
	(newThreadCard.data as any).tags = (topicCard.data as any).tags;
	...
	if (!threadCard) {
		return Object.assign({}, newThreadCard, {
			id: {
...
const isInternalEvent = (event: Card) => {
	if (isTopicEvent(event)) {
...
const getTopicFromPostUrl = async (
	context: Context,
	actor: string,
	options: any,
	url: string,
...
const getPostFromPostUrl = async (
	context: Context,
	actor: string,
	options: any,
	url: string,
		...
		> = remoteTopicData.post_stream
			? await Bluebird.map(remoteTopicData.post_stream.posts, async (post) => {
					...
					const item = Object.assign({}, post, {
						destroyed: Boolean(post.deleted_at),
						current:
							isPostEvent(event) && post.id === event.data.payload.post.id,

						actorId: await this.context.getActorId({
...
export const create = (options: SyncIntegrationOptions) => {
	return new DiscourseIntegration(options);
};
