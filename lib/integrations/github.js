...
module.exports = class GitHubIntegration {
	...
	async getOctokit (context, installationId) {
	...
	async getPRFromEvent (github, event, options) {
		...
		const pullRequest = event.data.payload.pull_request ||
			event.data.payload.issue.pull_request
	...
	async getRepoEvent (event, options) {
		const root = getEventRoot(event)
		const type = 'issue@1.0.0'
	...
	async getOrgCard (org, options) {
		const mirrorID = org.html_url
		const existingCard = await this.getCardByMirrorId(mirrorID, 'github-org@1.0.0')
		...
		const orgSlug = `github-org-${name}`.toLowerCase()
		const orgCard = {
			name: `${name}`,
			slug: orgSlug,
			type: 'github-org@1.0.0',
		...
		return {
			...
			card: makeCard(orgCard, options.actor)
		}
	}

	async createRepo (event, actor) {
		const targetRepo = event.data.payload.repository
		const result = []
		const repo = await this.getRepoCard(targetRepo, {
			actor,
		...
		if (targetRepo.owner.type === 'Organization') {
			const org = await this.getOrgCard(targetRepo.owner, {
				actor,
				index: 1
			})

			// If we created an org add it to the result and link it to the repo
			if (org.card) {
				result.push(org.card)
				const offset = result.length - 1
				result.push(
					makeCard({
						slug: `link-${repo.repoInfo.slug}-belongs-to-${org.card.slug}`,
						type: 'link@1.0.0',
						name: 'belongs to',
						data: {
							inverseName: 'has',
							from: {
								id: repo.repoInfo.target.id,
								type: 'repository@1.0.0'
							},
							to: {
								id: {
									$eval: `cards[${offset}].id`
								},
								type: 'github-org@1.0.0'
							}
						}
	...
	async translate (event) {
		...
		switch (type) {
			...
			case 'repository':
				return this.createRepo(event, actor)
